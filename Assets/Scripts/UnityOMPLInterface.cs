...
public delegate bool ClosestPointExternalDelegate(State state, ref State point);
...
public class UnityOMPLInterface
{
    ...
    private Func<State, bool> IsStateValidExtern;
    private ClosestPointExternalDelegate ClosestPointExtern;
    private Func<State> getState;
    private Action<State> restoreState;
    public UnityOMPLInterface(Func<State, bool> IsStateValidExternIn, ClosestPointExternalDelegate ClosestPointExternIn, Func<State> getStateIn, Action<State> restoreStateIn)
    {
    ...
    private bool IsStateValid(ref State state)
    {
        //State state = new State(stateStruct.x, stateStruct.y, stateStruct.theta);
        return IsStateValidExtern(state);
    }
    
    private bool GetClosestPoint(ref State stateStruct, ref State pointStruct)
    {
        //State state = new Vector3(stateStruct.x, stateStruct.y, stateStruct.theta);
        //Vector3 point = new Vector3();
         bool isPoint = ClosestPointExtern(stateStruct, ref pointStruct);
         //if (isPoint)
         //{
