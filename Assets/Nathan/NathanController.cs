...
namespace Nathan
{
    public class NathanController : MonoBehaviour
    {
        ...
        void Eat()
        {
            ...
            {
                agent.SetDestination(pre_eating_pos.position);
            }
            else if (loc == 2)
            {
                agent.SetDestination(eat_pos.position);
                animation_played = false;
            }
            else if (loc == 3)
            {
        ...
        void Update()
        {
            ...
            if (milliseconds > wait_until && Input.GetKey(KeyCode.S) && Input.GetKey(KeyCode.RightShift))
            {
                agent.SetDestination(eat_pos.position);
